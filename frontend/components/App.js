...
export default function App() {
  ...
  const [valuable,setVal]=useState(tialize)
  ...
  useEffect(() => {
    ...
    if(editing==null){
      ...
      const {fname,lname,bio}=members.find(ber=>ber.id==editing)
      setVal({fname,lname,bio})
    }
  }, [editing])
  ...
  const submitNewMember = () => {
    ...
    const {fname,lname,bio}=valuable
    const newest={fname,lname,bio,id:getId()}
    setMembers([...members,newest])
    setVal(tialize)
  }
  const editExistingMember = () => {
    ...
    setMembers(premem=>premem.map(mmm=>{
      if(mmm.id==editing){
        return {...mmm,...valuable}
      }
      return mmm
      ...
      <div id="membersForm">
        ...
        <form onSubmit={onSubmit}>
          <div>
            ...
            <input id="fname" type="text" placeholder="Type First Name" value={valuable.fname} onChange={onChange}/>
          </div>

          <div>
            <label htmlFor="lname">Last Name </label>
            <input id="lname" type="text" placeholder="Type Last Name" value={valuable.lname} onChange={onChange}/>
          </div>

          <div>
            <label htmlFor="bio">Bio </label>
            <textarea id="bio" placeholder="Type Bio" value={valuable.bio} onChange={onChange}/>
          </div>
